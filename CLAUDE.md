...
| Surface | Path | Who | Role | Front-end
| --- | --- | --- | --- |
| Book | `/book` | public / customer | Browse, provisional, pay the package confirm threshold to confirm | vue or livewire
| MyRental | `/myrental` | customer | That reservation until 3 days after `ends_at`; session and signed link; may request extend; remainder Connect | vue
| Terminal | `/terminal` | staff (password session) | Desk and floor POS: large buttons, speed over web convention; walk-in, assign/swap, pickup/return, extend, cash/other. Counter PCs and tablets. Device pairing later | vue
...
Pages live under `resources/js/pages/{Book,MyRental,Terminal,Cfd,Screen}/`. Terminal is a restaurant-style P.O.S.: large hit targets for one-finger tapping on a fixed counter screen or a tablet held in the other hand; few screens; trained speed — not a marketing or admin web layout. Book, MyRental, CFD, Screen, and Filament do not copy that chrome. A Reservation view lists **every** `bike_reservation` line: product, bid or unassigned, Bike situation, and which Reservation occupies that bike; scroll the list when the party does not fit. A non-`home` Bike situation always names its occupying Reservation (customer on Terminal, Filament, Book, MyRental, CFD). If a bike is shown on a ticket whose occupying Reservation is not that ticket, flag it as the wrong place. Do not collapse lines to a bid string or a qty. Screen still must not render customer PII: bid and situation only there.
