...
If a Reservation or Customer is viewed in Filament, show the same owed vs paid reminder. Reservation may write stage and owed via Actions. Still never take Connect charges or cash checkout in Shop Filament. Platform billing is not rental Connect.
...
v1 Shop Filament resources: Location (name, timezone, weekly hours, optional closed dates), BikeCategory, BikeModel, BikeModelVariant, Bike, RentalPackage (including variant rates), Staff, Devices, blocking service, Reservation (writable stage, owed, interval, and lines via Actions), Customer (read-only owed/paid reminder). Do not add Allocation. Hours are on Location, not a separate resource and not config-only. Filament still never captures Connect or cash checkout. Reservation writes must invoke domain Actions, not Eloquent in the resource. Pickup and return stay off Filament.
