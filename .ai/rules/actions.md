...
allocate and quoteOccupancy overlap [starts_at, ends_at] inclusive on the reservation for each bike_reservation row, then block until after ends_at plus the effective turnaround buffer. Book quoteOccupancy refuses if starts_at or ends_at is outside Location open hours or on a closed date; a closed night between them is allowed. Duration steps still affect the quote only. Terminal and Filament may set any interval. Pickup writes bike_reservation Out and Bike rented_out plus bike_situation_reservation_id together; return writes In and back or home. Terminal may set prepping or staged for that reservation before pickup; skip is allowed. Book does not write situation. Hours do not allocate.
...
quoteOccupancy is read-only. A bike with a stale Out is not available. Heal only in allocate, swap, assign (under row lock) or the scheduled tick, never an Out on a Cancelled reservation. Do not write tenant rows from quoteOccupancy.
...
## RefundAction; no customer self-refund
RefundAction records a refund transaction and recomputes paid. Connect charges refund through Stripe (application fee follows Stripe Connect refund). cash/other refunds are ledger rows only. Terminal and Filament Managers may invoke it. Book and MyRental must not self-refund. Do not release occupancy from a refund. Do not capture new charges in RefundAction.
