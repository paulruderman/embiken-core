...
/**
 * PROTOTYPE Variant M — Hour filmstrip. Each hour is a frame of bid chips (busy/free).
 * Timeline as cinema, thumb along the reel.
 */
...
function tap(bike: Bike, hour: number): void {
    const reservation = busyAt(shop, bike.id, hour);
    ...
    toast.value = `${bike.bid} free at ${hour}:00`;
}
...
<template>
    <div class="flex min-h-screen flex-col bg-zinc-950 pb-24 text-white">
        <header class="flex items-center gap-2 px-3 py-2">
            <span class="font-semibold">Filmstrip</span>
            <span class="font-mono text-xs">reel</span>
            <span class="ml-auto text-sm text-amber-300">{{ toast }}</span>
            <button type="button" class="h-14 rounded-2xl bg-yellow-400 px-4 font-bold text-zinc-950" @click="walkIn">
                Walk-in
            </button>
        </header>
        <div class="flex min-h-0 flex-1 gap-2 overflow-x-auto px-3">
            <section v-for="hour in hours" :key="hour" class="w-28 shrink-0">
                <h2 class="mb-1 text-center text-lg font-black">{{ hour }}</h2>
                <button
                    v-for="bike in shop.bikes"
                    :key="`${hour}-${bike.id}`"
                    type="button"
                    class="mb-1 h-10 w-full rounded-lg text-sm font-bold"
                    :class="busyAt(shop, bike.id, hour) ? 'bg-yellow-400 text-zinc-950' : 'bg-zinc-800 text-zinc-500'"
                    @click="tap(bike, hour)"
                >
                    {{ bike.bid }}
                </button>
        ...
        <aside v-if="focus" class="border-t border-zinc-800 p-3">
            ...
            <div class="mb-2 flex flex-wrap gap-1">
                <span v-for="line in focus.lines" :key="line.id" class="rounded-full bg-zinc-800 px-3 py-1">
            ...
            <div v-else-if="pane === 'assign'" class="flex flex-wrap gap-2">
                <button
                    ...
                    :key="bike.id"
                    type="button"
                    class="h-14 w-14 rounded-xl bg-zinc-800 font-bold"
                    @click="pickBike(bike)"
                >
                    {{ bike.bid }}
                </button>
            ...
            <div v-else class="grid grid-cols-4 gap-2">
                <button type="button" class="h-14 rounded-xl bg-zinc-800" @click="pane = 'assign'">Assign</button>
                ...
                <button type="button" class="h-14 rounded-xl bg-zinc-800" @click="acceptWaiver(focus)">Waiver</button>
                <button type="button" class="h-14 rounded-xl bg-zinc-800" @click="toast = focus.myrental">URL</button>
